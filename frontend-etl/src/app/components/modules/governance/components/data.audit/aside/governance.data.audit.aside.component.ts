...
export enum ActionTypeEnum {
    DETAIL = 'detail',              // 查看详情
    ADDTASK = 'addTask',           // 新建任务
    ADDCATALOG = 'addCatalog',      // 新建目录
    EDITCATALOG = 'editCatalog',              // 编辑目录
    EDITTASK = 'editTask'
}
@Component({
    selector: 'governance-data-audit-aside-component',
    templateUrl: './governance.data.audit.aside.component.html',
    styleUrls: ['./governance.data.audit.aside.component.scss']
})
export class GovernanceDataAuditAsideComponent implements OnInit {
    ...
    constructor(private modalService: ModalService,
        ...
        // 新增订阅
        this.datatransferService.dataAuditAddTreeSubject.subscribe(success => {
            if (success.flow) {
                ...
                if (this.updateFlow) {
                    ...
                    this.governanceService.getAuditDirectoryTree({parentId: this.updateFlow.id, catalogType: 'data'}).then(d => {
                        if (d.success) {
        ...
        // 树形删除、修改订阅
        this.datatransferService.dataAuditUpdateTreeSubject.subscribe(data => {
            if (data.flow.parentId === 0) {
                ...
                if (this.parentFlow) {
                    ...
                    this.governanceService.getAuditDirectoryTree({parentId: this.parentFlow.id, catalogType: 'data'}).then(d => {
                        if (d.success) {
                            d.message.catalog.forEach(item => {
    ...
    getTreeList(parentId: any) {
        this.governanceService.getAuditDirectoryTree({parentId: parentId, catalogType: 'data'}).then(d => {
            if (d.success && d.message) {
                ...
                if (parentId === 0) {
                    this.workflows = arr;
                    this.workflows[0].type === 'catalog' && this.getTreeList(this.workflows[0].id);
                } else {
    ...
    addDirectory() {
        let [ins] = this.modalService.toolOpen({
            ...
            datas: {
                type : ActionTypeEnum.ADDCATALOG
            },
            okCallback: () => {
                ins.okClick();
            }
