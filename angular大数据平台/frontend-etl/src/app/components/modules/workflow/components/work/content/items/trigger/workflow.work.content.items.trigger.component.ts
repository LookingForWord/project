...
@Component({
    selector: 'workflow-work-content-items-trigger-component',
    templateUrl: './workflow.work.content.items.trigger.component.html',
    styleUrls: ['./workflow.work.content.items.trigger.component.scss'],
    animations: [Animations.slideLeft]
})
export class WorkflowWorkContentItemsTriggerComponent implements OnInit {
    ...
    save() {
        ...
        this.workflowService.saveTrigger(params).then(d => {
            if (d.rspcode === ServiceStatusEnum.SUCCESS) {
               this.modalService.alert('触发器添加成功', {time: 1500});
               this.task.flowInfo.flow.timing = str;
               this.task.updating = true;
               this.cancle();
            } else {
                this.modalService.alert('触发器添加失败');
            }
        });
    }
